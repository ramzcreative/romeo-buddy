...
- **Typography**: `--fs-xs` through `--fs-xxxl` (sizes, also fluid), `--fw-light` through `--fw-heavy` (weights), `--lh-xs` through `--lh-xl` (line heights), `--ls-xs` through `--ls-xl` (letter spacing), `--font-display` (`"Baloo Bhaina 2"`) / `--font-body` (`"ABeeZee"`) — generated into `generated/font-stack-generated.pcss`, managed via the Theme Designer CP's Font Stack tab, not hand-authored. `--font-accent` has no family assigned here (only 2 uploaded font families, unlike `stables`' 2 that both get used).
