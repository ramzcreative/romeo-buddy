...
| `js/Components/` | Native Web Components (`class X extends HTMLElement`) for interactive UI — `modal.ts`, `accordion.ts`, `cookieConsent.ts`, `videoPlayer.ts`, the `effects/` a Theme variant switches on (`snowFall.ts`, imported only when its element is on the page), plus the `motion/` subsystem (scroll/inview transitions used by `[data-motion]` blocks). |
...
- **Spacing**: `--spacer-1` through `--spacer-9`, each a fluid `clamp()` (scales with viewport width — no separate mobile/desktop values to maintain). Custom **pairs** (`--spacer-4-6`, `--spacer-7-8`, `--spacer-7-9`, `--spacer-8-10`, ...) span from one step's min to another's max, giving a steeper ramp than any single step — created in the Theme Designer, so check what this site actually has before reaching for one. Section rhythm reads through `--section-space` (see `base/layouts.pcss`), never a raw spacer.
- **Typography**: `--fs-xs` through `--fs-4xl` (9 fluid steps), `--fw-light` through `--fw-heavy` (weights), `--lh-impact`/`-tight`/`-normal`/`-loose` (line heights), `--ls-tight`/`-normal`/`-loose`/`-wide` (letter spacing) — all generated into `generated/scale-generated.pcss`, managed via the Theme Designer CP's Type Scale tab, not hand-authored. `--font-display` (`"Baloo Bhaina 2"`) / `--font-body` (`"ABeeZee"`) and the real per-selector CSS for each of the 13 text styles (h1-h6, subheading, preheading, paragraph + 3 size variants, links) are generated into `generated/font-stack-generated.pcss`, managed via the Theme Designer CP's Font Stack tab. `--font-accent` has no family assigned here (only 2 uploaded font families, unlike `stables`' 2 that both get used).
