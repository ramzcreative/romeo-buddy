//https://github.com/nathansmith/cta-modal?tab=readme-ov-file
// ===========================
...
if ('customElements' in window) {
    ...
    <style>
      ...
      *:before {
      ...
      .cta-modal__overlay {
        ...
        display: flex;
        align-items: center;
        justify-content: var(--cta-modal-overlay-justify, center);
        ...
        width: 100%;
        height: var(--cta-modal-overlay-height, auto);
        min-height: var(--cta-modal-overlay-min-height, 100%);
      }
      ...
      .cta-modal__dialog {
        ...
        height: var(--cta-modal-dialog-height, initial);
        max-width: 100%;
      ...
      [${DATA_SHOW}='true'] .cta-modal__dialog {
        animation-duration: ${ANIMATION_DURATION}ms;
        animation-name: var(--cta-modal-show-animation, SHOW-DIALOG);
      }
      ...
      [${DATA_HIDE}='true'] .cta-modal__dialog {
        animation-duration: ${ANIMATION_DURATION}ms;
        animation-name: var(--cta-modal-hide-animation, HIDE-DIALOG);
        transform: scale(0.95);
      }
  ...
  class CtaModal extends HTMLElement {
    ...
    static get observedAttributes() {
      return [ACTIVE, ANIMATED, CLOSE, STATIC, DOM_OPEN];
    }
...
_addEvents() {
    ...
    // Access the class attribute from the host element
    const hostClasses = this.getAttribute('class');
    if (hostClasses) {
      // Apply the classes to an internal element (e.g., the div in the shadow DOM)
     // this._shadow.querySelector('div').classList.add(...hostClasses.split(' '));
    }
...
// ========================
// Helper: set dom open flag.
// ========================

_setDomOpenFlag() {
    this._isDomOpen = this.getAttribute(DOM_OPEN) === TRUE;
}
