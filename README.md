...
npm run build
```
This is the one command for everything a site needs to go live: one Vite build of every theme's CSS and the shared JS into `web/dist/site/`, optimized icons (`svg-build`), and each theme's favicons and logo (`favicon-build`, `logo-build`) — see [Themes](#themes) below. Staging and production are identical here: both set `CRAFT_DEV_MODE=false`, so there's no separate "staging build" — the same output serves either, only the deployed `.env` differs.
...
- `src/css/main.pcss` / `critical.pcss` — import the theme's own `generated/colors-generated.pcss` first, then delegate to `_base/src/css/main.pcss` / `critical.pcss`; `critical.pcss` also imports this theme's own other `generated/` files after that, buttons and backgrounds included, so buttons and section backgrounds are styled on the first frame, and its `fonts-generated.pcss`: `@font-face` for only the families the theme uses, from the one font pool on the Theme Designer's Fonts tab (files in `web/fonts/`, named with a content hash)
...
- `templates/` forks and `src/css/blocks/` — one theme's own version of a block: the template copied under its own BEM block name, its CSS imported in `layer(overrides)` (from `critical.pcss` if the block can sit above the fold)
...
For a **palette-only variant** (the common case): copy an existing theme's *files*, not its content — `cp -r themes/christmas themes/<handle>` gives you the right minimal skeleton (its own `generated/` output + thin `main.pcss`/`critical.pcss`/`critical.js`/`maincss.js` + `templates/`), then just edit `src/css/generated/colors-generated.pcss` with the new palette.

For anything more involved:
1. Set up `themes/<handle>/src/` with its own `generated/colors-generated.pcss`, thin `main.pcss` / `critical.pcss` files that import `_base`'s equivalents, and the `critical.js` / `maincss.js` wrappers (copy the pattern from `themes/christmas/src/`)
...
5. Run `php craft theme-designer/fonts/sync` if the theme was added by hand, so it gets its fonts file and import
